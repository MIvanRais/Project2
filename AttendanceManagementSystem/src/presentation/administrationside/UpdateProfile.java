        ...
        if (e.getSource() == btnSave) {
            if (textFieldFirstName.getText().trim().isEmpty() || textFieldLastName.getText().trim().isEmpty()
                ...
                if (LogicBuilding.updateProfile(
                    "UPDATE admin_details SET first_name = '" + UpdateProfile.getFirstName + "', last_name = '"+ UpdateProfile.getLastName +"', gender = '"+ UpdateProfile.getGender +"', no_telephone = '"+ UpdateProfile.getNoTlphn +"', email = '"+ UpdateProfile.getEmail +"' WHERE admin_username = '"+ Login.username +"';"
                    )
                    ) {
